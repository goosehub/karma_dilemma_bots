<?php
/*
Selfless Bottom Feeder
This simple bot will make a low bid on all game auctions
On games, it makes whichever choice has the bigger potential payoff for the other player
*/
...
function play_games() {
	...
	foreach ($response->started_games as $game) {
		...
		// Find the choice that has the best potential payoff for the other player
...
function send_request($path, $data) {
	// Info for debug
	echo 'Calling ' . $path . ': ' . print_r($data, true) . '<br>' . PHP_EOL;
	...
	// Catch error
	if ($response->error) {
		echo $response->error_code . ' - ' . $response->error_message . '<br>' . PHP_EOL;
	}
